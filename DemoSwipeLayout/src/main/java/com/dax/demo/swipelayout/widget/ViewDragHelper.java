...
public class ViewDragHelper {
    ...
    private double mDragHorizontalAngle;

    private double mDragVerticalAngle;
    ...
    private boolean isAllAreaCanScroll = false;
     ...
     * callback's
     * {@link com.kugou.android.app.swipeback.ViewDragHelper.Callback#onEdgeTouched(int, int)}
     * and
     * {@link com.kugou.android.app.swipeback.ViewDragHelper.Callback#onEdgeDragStarted(int, int)}
     ...
     * {@link com.kugou.android.app.swipeback.ViewDragHelper.Callback#tryCaptureView(android.view.View, int)}
            ...
            case MotionEvent.ACTION_MOVE: {
                if (mDragState == STATE_DRAGGING) {
                    ...
                    if (Math.abs(idx) > Math.abs(idy * 2)) {
                        dragTo(mCapturedView.getLeft() + idx, mCapturedView.getTop() + idy, idx,
                                idy);
                    }
                    saveLastMotion(ev);
                } else {
    ...
    private void dragTo(int left, int top, int dx, int dy) {
        ...
        if (dx != 0 || dy != 0) {
            ...
            if (clampedDy == 0) {
                ...
                double angle = Math.abs(clampedDy) / Math.abs(clampedDx);
                if (angle < mDragVerticalAngle) {
                    mCallback.onViewPositionChanged(mCapturedView, clampedX, clampedY, clampedDx,
                            clampedDy);
                }
            }
     ...
     * {@link com.kugou.android.app.swipeback.ViewDragHelper.Callback#getOrderedChildIndex(int)}
     * .
     * 
     * @param x X position to test in the parent's coordinate system
     * @param y Y position to test in the parent's coordinate system
    ...
    private int getEdgesTouched(int x, int y) {
        ...
        if (isAllAreaCanScroll || x < mParentView.getLeft() + mEdgeSize)
            result |= EDGE_LEFT;
        if (isAllAreaCanScroll || y < mParentView.getTop() + mEdgeSize)
            result |= EDGE_TOP;
        if (isAllAreaCanScroll || x > mParentView.getRight() - mEdgeSize)
            result |= EDGE_RIGHT;
        if (isAllAreaCanScroll || y > mParentView.getBottom() - mEdgeSize)
            result |= EDGE_BOTTOM;

        return result;
    }

    public double getDragHorizontalAngle() {
        return mDragHorizontalAngle;
    }

    public void setDragHorizontalAngle(double pDragAngle) {
        this.mDragHorizontalAngle = pDragAngle;
    }

    public double getDragVerticalAngle() {
        return mDragVerticalAngle;
    }

    public void setDragVerticalAngle(double mDragVerticalAngle) {
        this.mDragVerticalAngle = mDragVerticalAngle;
    }

    public boolean isAllAreaCanScroll() {
        return isAllAreaCanScroll;
    }

    public void setAllAreaCanScroll(boolean isAllAreaCanScroll) {
        this.isAllAreaCanScroll = isAllAreaCanScroll;
    }
}
